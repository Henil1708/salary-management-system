...
- **Salaries are monthly, and `department` is a DB-backed reference (not a static enum)** — a new department is immediately assignable; employee/CSV `department` is validated against the DB. Country and job level remain static enums in `shared/`.
- **Payroll runs are a scope extension (payment tracking, not disbursement).** A run snapshots each active employee's current salary into immutable line items tracked Pending → Paid; run status is derived from item counts; "mark all paid" respects the active item filter. It records payment status only — no money movement/tax/bank rails (still out of scope). Salary *management* (the Salaries area) and *payroll* coexist as separate concerns. See `docs/TRADEOFFS.md` §10.
- **Import/export are contextual actions, not a standalone feature.** CSV import (of employee+salary records) and CSV export of the current filtered view are surfaced as actions inside the section they belong to (the Salaries / Employees management screens), not as a separate top-level "Bulk Import" tab. The server endpoints (`/import`, `/export`) already exist; the UI hangs off the relevant page.
